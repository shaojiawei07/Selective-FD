...
def generate_softlabel(args, model_list, device, global_dataset, batch_size):
    ...
    with torch.no_grad():
        ...
        mini_batch_hardlabel = 0.0
        for j in range(10):
            model_list[j].eval()
            _, softlabel, _ = model_list[j](mini_batch_data.to(device))


            pred = softlabel.argmax(dim=1, keepdim=False)
            hardlabel = torch.zeros((batch_size, 10))

            for k in range(batch_size):
                hardlabel[k, pred[k]] += 1

            #mini_batch_softlabel += softlabel * torch.reshape(mini_batch_ensemble_label[:,j], (-1,1) ).to(device) #/ 10.0
            mini_batch_hardlabel += hardlabel.to(device) * torch.reshape(mini_batch_ensemble_label[:,j], (-1,1) ).to(device) / 10.0

    return mini_batch_data, mini_batch_hardlabel
